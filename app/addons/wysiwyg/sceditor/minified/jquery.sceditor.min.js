    ...
    $.sceditor = function (el, options) {
        ...
        initEvents = function() {
            ...
            $wysiwygBody
                ...
                .on('keyup', appendNewLine)
                .on('blur', valueChangedBlur)
                .on('keyup', valueChangedKeyUp)
                .on('paste', handlePasteEvt)
                .on(IE_VER ? 'selectionchange' : 'keyup focus blur contextmenu mouseup touchend click', checkSelectionChanged)
                .on('keydown keyup keypress focus blur contextmenu', handleEvent);
        
            if(options.emoticonsCompat && window.getSelection)
                $wysiwygBody.on('keyup', emoticonsCheckWhitespace);
            ...
            $wysiwygDoc
                ...
                .on('blur', valueChangedBlur)
                .on(IE_VER ? 'selectionchange' : 'focus blur contextmenu mouseup click', checkSelectionChanged)
                .on('beforedeactivate keyup', saveRange)
                .on('keyup', appendNewLine)
                .on('focus', function() {
                    lastRange = null;
                });
...
// TODO: Make this tag configurable.
        ...
        handleCommand = function (caller, command) {
            ...
            if(base.inSourceMode())
            {
                if(command.txtExec)
                {
                    if(Array.isArray(command.txtExec))
...
// TODO: Make configurable
    ...
    $.sceditor.commands = {
        ...
        font: {
            _dropDown: function(editor, caller, callback) {
                ...
                for (var i=0; i < fonts.length; i++)
                    content.append(_tmpl('fontOpt', {font: fonts[i]}, true).on('click', clickFunc));
        ...
        size: {
            _dropDown: function(editor, caller, callback) {
                ...
                for (var i=1; i<= 7; i++)
                    content.append(_tmpl('sizeOpt', {size: i}, true).on('click', clickFunc));
        ...
        emoticon: {
            exec: function (caller) {
                ...
                var createContent = function(includeMore) {
                    ...
                    {
                        $content.append(
                            $(editor._('<a class="sceditor-more">{0}</a>', editor._('More'))).on('click', function () {
    ...
    /** @lends jQuery.sceditor.dom */
    {
        ...
        hasStyle: function (elm, property, values) {
            ...
            if (!styleValue)
                return (!values || styleValue === values || (Array.isArray(values) && $.inArray(styleValue, values) > -1));
